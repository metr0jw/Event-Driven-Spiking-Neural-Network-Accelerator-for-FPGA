...
typedef struct {
    ...
    XSnn_top_hls_uio_map maps[ MAX_UIO_MAPS ];
} XSnn_top_hls_uio_info;

/***************** Variable Definitions **************************************/
static XSnn_top_hls_uio_info uio_info;
...
static int uio_info_read_name(XSnn_top_hls_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSnn_top_hls_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSnn_top_hls_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSnn_top_hls_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSnn_top_hls_Initialize(XSnn_top_hls *InstancePtr, const char* InstanceName) {
	XSnn_top_hls_uio_info *InfoPtr = &uio_info;
